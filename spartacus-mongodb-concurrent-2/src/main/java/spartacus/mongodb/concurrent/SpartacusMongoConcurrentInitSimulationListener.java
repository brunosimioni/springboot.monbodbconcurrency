...
@Configuration
public class SpartacusMongoConcurrentInitSimulationListener implements ApplicationListener<EmbeddedServletContainerInitializedEvent> {
	...
	public void onApplicationEvent(EmbeddedServletContainerInitializedEvent event) {
		...
		for (long i = 0; i < total; i++) {
			...
			mongoTemplate.findAndModify(
				...
				Update.update("counter2", (item.getCounter2()+1)),
				ConcurrentItem.class
			);

			
			item.getCounters2().add(i);
			mongoTemplate.findAndModify(
				new Query(Criteria.where("_id").is("1")), 
				Update.update("counters2", item.getCounters2()),
				ConcurrentItem.class
			);
